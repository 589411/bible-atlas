...
4. `node verify_quotes.js` → 抓 bolls 對金句逐字硬檢查,**🔴(金句/key 行)必須 0**。
   - 🔴 = 金句非原文「連續子字串」:**金句務必取單一連續片段,嚴禁跨子句拼接**(跳掉中間子句即算拼接);用字以和合本為準(舐≠舔、現今我≠我今、勿自行加「許願」)。
   - 🟡 = 支援台詞某子句不符:可接受刪掉整個連接子句,但保留的子句不可改字/杜撰。
   - 驗證器已自動正規化和合本慣用異體字(裡/裏、啊/阿、哪/那、嗎/麼、吧/罷)與剝除 bolls 編註「（原文是…）」,故這類差異不算錯。
5. `node scripts/validate.js` → 確認時代資料未被破壞(仍 `驗證通過`)。
6. 若新增需離線快取的檔案才動 `sw.js`(加入 SHELL 並升 `CACHE` 版本);
   一般只改 `data/highlights.json` 不必動 sw.js(network-first 會更新)。
7. commit & push;atlas.launchdock.app 自動部署。
